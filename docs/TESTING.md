...
every naming convention renders correctly. There are no automated tests yet — this
is the eyeball-against-spec pass that replaces them.
...
| G7 | Raw bytes serve at `/directory/<path>` (open an image's `/directory/...` URL directly) | ⬜ | |
...
| F4 | Featured targets are EXCLUDED from the plain file/folder listing | ⬜ | |
...
| # | Check | Status | Notes |
|---|---|---|---|
| M1 | `/markdown_cheat_sheet.txt` detail view renders formatted HTML, not raw source | ⬜ | |
